...
int main(void) {
  ...
  TwAddVarRW(QuaternionGUI, "Use LookAt", TW_TYPE_BOOL8, &gLookAtOther,
             "help='Look at the other monkey ?'");

  // Set GLFW event callbacks. I removed glfwSetWindowSizeCallback for
  // conciseness
  glfwSetMouseButtonCallback(
      window, (GLFWmousebuttonfun)
                  TwEventMouseButtonGLFW); // - Directly redirect GLFW mouse
                                           // button events to AntTweakBar
  glfwSetCursorPosCallback(
      window,
      (GLFWcursorposfun)TwEventMousePosGLFW); // - Directly redirect GLFW mouse
                                              // position events to AntTweakBar
  glfwSetScrollCallback(
      window,
      (GLFWscrollfun)TwEventMouseWheelGLFW); // - Directly redirect GLFW mouse
                                             // wheel events to AntTweakBar
  glfwSetKeyCallback(
      window,
      (GLFWkeyfun)
          TwEventKeyGLFW); // - Directly redirect GLFW key events to AntTweakBar
  glfwSetCharCallback(
      window, (GLFWcharfun)TwEventCharGLFW); // - Directly redirect GLFW char
                                             // events to AntTweakBar
  ...
  const GLuint vertexPosition_modelspaceID =
      glGetAttribLocation(programID, "vertexPosition_modelspace");
  const GLuint vertexUVID = glGetAttribLocation(programID, "vertexUV");
  const GLuint vertexNormal_modelspaceID =
  ...
  model_ns::model my_model{"suzanne.obj"};
  do {
    ...
    { // Euler
      ...
      const glm::mat4 RotationMatrix =
          eulerAngleYXZ(gOrientation1.y, gOrientation1.x, gOrientation1.z);
      const glm::mat4 TranslationMatrix =
          translate(mat4(), gPosition1); // A bit to the left
      const glm::mat4 ScalingMatrix = scale(mat4(), vec3(1.0f, 1.0f, 1.0f));
    ...
    { // Quaternion
      ...
      if (gLookAtOther) {
        const vec3 desiredDir = gPosition1 - gPosition2;
        const vec3 desiredUp = vec3(0.0f, 1.0f, 0.0f); // +Y

        // Compute the desired orientation
        const quat targetOrientation = normalize(LookAt(desiredDir, desiredUp));

        // And interpolate
        gOrientation2 =
            RotateTowards(gOrientation2, targetOrientation, 1.0f * deltaTime);
      }

      const glm::mat4 RotationMatrix = mat4_cast(gOrientation2);
      const glm::mat4 TranslationMatrix =
          translate(mat4(), gPosition2); // A bit to the right
      const glm::mat4 ScalingMatrix = scale(mat4(), vec3(1.0f, 1.0f, 1.0f));
      const glm::mat4 ModelMatrix =
          TranslationMatrix * RotationMatrix * ScalingMatrix;
